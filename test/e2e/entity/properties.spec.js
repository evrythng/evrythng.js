...
module.exports = (scopeType, targetType) => {
  ...
  describe(`Properties (${targetType})`, () => {
    ...
    it(`should create a ${targetType} property`, async () => {
      ...
      const res = await scope[targetType](target.id).property().create(payload)
    ...
    it(`should read all ${targetType} properties`, async () => {
      const res = await scope[targetType](target.id).property().read()
    ...
    it(`should read a single ${targetType} property`, async () => {
      const res = await scope[targetType](target.id).property(PROPERTY_NAME).read()
    ...
    it(`should update a single ${targetType} property`, async () => {
      const res = await scope[targetType](target.id).property(PROPERTY_NAME).update(PROPERTY_VALUE + 1)
    ...
    if (scopeType !== 'anonUser') {
      it(`should delete a single ${targetType} property`, async () => {
        await scope[targetType](target.id).property(PROPERTY_NAME).delete()
      })
    }
  })
}

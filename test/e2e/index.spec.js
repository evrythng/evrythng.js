...
describe('e2e tests for apiVersion=2', () => {
  describe('as Operator', () => {
    ...
    it('should create operator access and read it', async function test () {
      ...
      const findAdminAccountPolicy = await operator.accessPolicy().setFilter('name=evt:accountAdmin').read()
      ...
      const createdOperarorAccess = await operator.sharedAccount(ACCOUNT_ID).operatorAccess().create(operatorData)
      assert.deepInclude(createdOperarorAccess, operatorData, 'Incorrect operatorAccess is created')

      const readOperatorAccess = await operator.sharedAccount(ACCOUNT_ID).operatorAccess(createdOperarorAccess.id).read()
      assert.equal(readOperatorAccess.id, createdOperarorAccess.id, `Can not read operator access by ${createdOperarorAccess.id}`)
    })
    ...
    it('should get your own access', async function () {
      ...
      assert.equal(myAccess.permissions, adminAccountPolicy.permissions, 'Can not get my own access')
    })
  })
